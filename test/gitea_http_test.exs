defmodule Gitea.HttpTest do
  ...
  test "giteaHttp.parse_body_response({:error, err})" do
    assert Gitea.Http.parse_body_response({:error, "err"}) == {:error, "err"}
  end

  # We've seen an empty body returned in practice. But how to test it ...?
  test "giteaHttp.parse_body_response({:ok, response}) with empty/nil body" do
    res = %{body: ""}
    assert Gitea.Http.parse_body_response({:ok, res}) == {:error, :no_body}
  end

  test "giteaHttp.get/1 gets (or mocks) an HTTP GET request to gitea Server" do
    repo_name = "public-repo"
    url = "https://gitea-server.fly.dev/api/v1/repos/myorg/#{repo_name}"
